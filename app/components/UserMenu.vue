...
onMounted(async () => {
  if (isAuthenticated) {
    userInfo.value = {
      name: userStore.value.user.name || 'Not logged in',
      username: userStore.value.user.username
    }
    ...
    user.value.name = userStore.value.user.name || 'Not logged in'
