...
const { data } = await useFetch<Qualification[]>(
  `/api/qualifications/${route.params.submissionId}`,
  {
    query: withYearQuery()
  }
)

type QualificationTabItem = Qualification & {
  label: string
  ChoiceRank: number | string
}

const getShortSchoolName = (school: string) => {
  return school === 'Foreign Language Immersion and Cultural Studies School'
    ? 'FLICS'
    : school === 'Palmer Park Preparatory Academy'
      ? 'Palmer'
      : school === 'Chrysler Elementary'
        ? 'Chrysler'
        : school === 'Edmonson Elementary'
          ...
          : school === 'Edison Elementary'
            ...
            : school === 'Bates Academy'
              ? 'Bates'
              : school === 'The School at Marygrove'
                ? 'Marygrove'
                : school
}

const qualificationItems = computed<QualificationTabItem[]>(() => {
  return (data.value || []).map((qualification) => ({
    ...qualification,
    label: getShortSchoolName(qualification.School),
...
<template>
  <UDashboardPanel
    :id="
      Array.isArray($route.params.submissionId)
  ...
  >
    ...
    <template #body>
      <UModal
        ...
        title="Add New Placement Result"
        description="Add a new placement result for the selected student"
      ...
      >
        ...
        <template #body>
          <UForm :schema="schema" :state="state">
            Perform the "{{ formValues.actionLong }}" action for {{ formValues.FullName }} at
            ...
            <div v-if="actions.pendingChanges.value.length > 0">
              ...
              <ol class="text-left list-decimal list-inside mt-2 mb-4">
                <li
                  v-for="(change, index) in actions.pendingChanges.value"
                  :key="`${index}-${change}`"
                  class="mb-2"
                >
                  {{ change }}
        ...
        <template #footer>
          ...
          <UButton
            ...
            @click="
              actions.addResult(
                { ...formValues, stage: actions.buttonText.value },
      ...
      <div class="">
        ...
        <UTabs :items="qualificationItems" orientation="horizontal" :ui="{ list: 'w-full' }">
          <template #content="{ item }">
            <div class="flex flex-col gap-2">
              ...
              <div
                v-if="[1084].includes(item.SchoolID) && ['2', '3'].includes(item.GradeEntering)"
            ...
            <div
              v-if="
                eligible.includes(item.EligibilityStatus) && ![0, 1, 2, 3].includes(item.ChoiceRank)
              "
            >
              <UButton
                class="mt-2"
                label="Add to placement results"
                @click="
                  loadItem({
                    ...item,
                    actionLong: 'Add Placement Result'
                  } as Qualification)
                "
              />
            </div>
